...
function wc_cart_totals_shipping_total_vendedor($nome_vendedor) {
	...
	foreach ( $packages as $i => $package ) {
		...
		$vendedor_atual = wp_kses_post(apply_filters( 'woocommerce_shipping_package_name', ( ( $i + 1 ) > 1 ) ? sprintf( _x( 'Shipping %d', 'shipping packages', 'woocommerce' ), ( $i + 1 ) ) : _x( 'Shipping', 'shipping package', 'woocommerce' ), $i, $package ));
		...
		//echo $vendedor_atual;
		$vendedor_atual = str_replace(' Entrega', '', $vendedor_atual);

		if($vendedor_atual == $nome_vendedor){
			$method = current( $package['rates'] );
			echo wc_price(wc_cart_totals_shipping_method_label_total( $method ));
		}
		
		//$frete_total = $frete_total+wc_cart_totals_shipping_method_label_total( $method );

		
	}

	//echo $nome_vendedor;
	//echo wc_price($frete_total);
	//var_dump($frete_total);
}



function wc_cart_totals_shipping_total() {
	$packages = WC()->shipping->get_packages();
	$first    = true;
	...
	foreach ( $packages as $i => $package ) {
		...
		wc_get_template(
			'cart/cart-shipping-total.php', array(
				'package'                  => $package,
...
function wc_cart_totals_shipping_method_label_total( $method ) {
	//$label = $method->get_label();
	$label = '';

	if ( $method->cost >= 0 && $method->get_method_id() !== 'free_shipping' ) {
		if ( WC()->cart->display_prices_including_tax() ) {
			$label .= $method->cost + $method->get_shipping_tax();
			if ( $method->get_shipping_tax() > 0 && ! wc_prices_include_tax() ) {
				$label .= ' <small class="tax_label">' . WC()->countries->inc_tax_or_vat() . '</small>';
			}
		} else {
