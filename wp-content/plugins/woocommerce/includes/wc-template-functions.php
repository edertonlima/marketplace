		...
		switch ( $args['type'] ) {
			case 'country':
				...
				if ( 1 === count( $countries ) ) {
					...
					$field = '<span class="select"><select name="' . esc_attr( $key ) . '" id="' . esc_attr( $args['id'] ) . '" class="country_to_state country_select ' . esc_attr( implode( ' ', $args['input_class'] ) ) . '" ' . implode( ' ', $custom_attributes ) . '><option value="">' . esc_html__( 'Select a country&hellip;', 'woocommerce' ) . '</option>';
			...
			case 'state':
				...
				if ( is_array( $states ) && empty( $states ) ) {
					...
					$field .= '<span class="select"><select name="' . esc_attr( $key ) . '" id="' . esc_attr( $args['id'] ) . '" class="state_select ' . esc_attr( implode( ' ', $args['input_class'] ) ) . '" ' . implode( ' ', $custom_attributes ) . ' data-placeholder="' . esc_attr( $args['placeholder'] ) . '">
...
function wc_empty_cart_message() {
	echo '<div class="block-produto cart"><span class="title-block">Carrinho Vazio :(</span><p class="sub-titulo">Seu carrinho está vazio no momento.</p></div>';
}
